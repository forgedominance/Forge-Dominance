...
function validateOrderForm(data) {
  ...
  // Address is not required to send an inquiry (this is a share/contact
  // flow, not a real checkout) — collected if provided, but won't block.
  return { ok: true };
}
...
async function submitOrderLead() {
  // Validation must still block share — we need a real name/email/address
  // to put in the WhatsApp/email message.
  ...
  // not a real checkout. It must NEVER block the actual share action
  // (mailto / WhatsApp) or the success screen — if /api/orders/public is
  // missing, down, or errors out, we still want the inquiry to go through.
  try {
    ...
    // Log only — do not alert, do not block. This is not a checkout flow.
    console.warn('Order lead backend save failed (non-blocking):', error);
  }
  return true;
}
...
async function shareOrderWhatsApp() {
  ...
  const action = async () => {
    ...
    // Show the confirmation FIRST, synchronously — same reasoning as email.
    // wa.me links can hijack the current tab on some mobile browsers/apps
    // instead of cleanly opening a new one, which can interrupt anything
    // scheduled to run afterward. Committing the confirmation first means
    // it's already done regardless of what the WhatsApp navigation does.
    showOrderSuccess();
...
async function shareOrderEmail() {
  ...
  const action = async () => {
    ...
    const mailtoUrl = `mailto:${site.contactEmail || 'forgedominance@gmail.com'}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;

    // Show the confirmation FIRST, synchronously, before touching mailto.
    // Clicking a mailto link can pop an OS "choose an email app" dialog,
    // which can blur/pause the tab and silently drop anything scheduled
    // with setTimeout afterward. Doing the confirmation first means it's
    // already committed regardless of what the mailto click does next.
    showOrderSuccess();

    // A detached <a> click is not treated as real page navigation, so
    // unlike window.location.href it can't abort this script if the
    // browser/OS has no mailto handler registered.
    try {
...
function showOrderSuccess() {
  ...
  try {
    ...
    sessionStorage.setItem('fd_order_sent_signature', getCartSignature(items));
  } catch (e) {}
  clearCart();
  document.body.style.overflow = '';
  ...
  // Swap the DOM synchronously, in this same tick — no nested setTimeout.
  // A queued timer here can get silently killed if the mailto/WhatsApp
  // action that triggered this causes the browser to blur/pause the tab
  // (e.g. "choose an email app" dialog), leaving the cart cleared but the
  // form still visible. Doing it immediately removes that race entirely.
  wrap.style.setProperty('display', 'none', 'important');
  success.classList.add('show');
  success.style.setProperty('display', 'flex', 'important');
  success.style.setProperty('flex-direction', 'column', 'important');
  success.style.setProperty('align-items', 'center', 'important');
  success.style.setProperty('justify-content', 'center', 'important');
  success.style.setProperty('min-height', '50vh', 'important');
  success.style.setProperty('width', '100%', 'important');
  success.style.setProperty('opacity', '1', 'important');
  success.style.setProperty('visibility', 'visible', 'important');
  success.style.setProperty('position', 'relative', 'important');
  success.style.setProperty('z-index', '10', 'important');
  success.style.setProperty('transform', 'none', 'important');
  window.scrollTo(0, 0);
  success.scrollIntoView({ behavior: 'smooth', block: 'start' });
  // Defensive re-assert on the next frame, in case any interruption
  // (tab blur from mailto/wa.me, another script) raced with the change
  // above. Cheap and idempotent if everything already applied correctly.
  requestAnimationFrame(() => {
    wrap.style.setProperty('display', 'none', 'important');
    success.classList.add('show');
    success.style.setProperty('display', 'flex', 'important');
...
function showNewOrderConfirmation() {
  // Brand-new, self-contained confirmation banner. Built entirely with
  // inline styles set via JS so it cannot be hidden or broken by any
  // existing CSS rule, class conflict, or stale markup on the page.
  const existing = document.getElementById('fd-new-order-confirmation');
  if (existing) existing.remove();

  const banner = document.createElement('div');
  banner.id = 'fd-new-order-confirmation';
  banner.setAttribute('style', [
    ...
    // This site hides the real system cursor everywhere (it uses a custom
    // cursor-follower element instead), which is why the mouse pointer was
    // invisible on the banner. Force the standard OS-default arrow cursor
    // back on for this banner and everything inside it.
    'cursor: default'
  ].join(';'));

  const icon = document.createElement('div');
  icon.textContent = '✓';
  icon.setAttribute('style', 'width:4.5rem;height:4.5rem;border:2px solid #D4500A;border-radius:50%;display:flex;align-items:center;justify-content:center;color:#D4500A;font-size:2rem;margin-bottom:1.75rem;cursor:default;');

  const heading = document.createElement('h2');
  heading.textContent = 'Message Sent.';
  heading.setAttribute('style', 'font-size:2.5rem;margin:0 0 1rem;color:#F2F0EC;cursor:default;');

  const message = document.createElement('p');
  message.textContent = 'Your order details have been sent. Our team typically responds within 2 hours to confirm specifications, timeline, and next steps. No payment is required until your build is confirmed.';
  message.setAttribute('style', 'font-size:1rem;color:#9A9A9A;max-width:32rem;line-height:1.7;margin:0 0 1.5rem;cursor:default;');

  const closeBtn = document.createElement('button');
  closeBtn.textContent = 'Close';
  closeBtn.setAttribute('style', 'background:#D4500A;color:#fff;border:none;padding:0.9rem 2rem;font-size:1rem;cursor:pointer;border-radius:0.25rem;');
  closeBtn.onclick = () => {
    banner.remove();
    // Reset the order form back to a blank state so it's ready to be
    // filled out again if the person navigates back to it later.
    const wrap = document.getElementById('order-form-wrap');
    const success = document.getElementById('order-success');
    ...
    if (success) {
      ...
      success.style.removeProperty('visibility');
      success.style.removeProperty('position');
      success.style.removeProperty('z-index');
      success.style.removeProperty('transform');
      success.style.removeProperty('flex-direction');
      success.style.removeProperty('align-items');
      success.style.removeProperty('justify-content');
      success.style.removeProperty('min-height');
      success.style.removeProperty('width');
    }
    if (form) form.reset();
    try {
      sessionStorage.removeItem('fd_order_sent');
      sessionStorage.removeItem('fd_order_sent_signature');
    } catch (e) {}
...
function getCartSignature(items) {
  // Order-independent signature of a cart's contents, so we can tell
  // "same order re-appearing" apart from "genuinely new/different items".
  return items
    .map(item => [item.name, item.steel || '', item.url || '', item.price, item.qty || 1].join('|'))
    .sort()
    .join('||');
}
function restoreOrderSuccessIfSent() {
  try {
    if (sessionStorage.getItem('fd_order_sent') === '1') {
      const items = loadCartState();
      const sentSignature = sessionStorage.getItem('fd_order_sent_signature') || '';
      const currentSignature = getCartSignature(items);
      if (items.length > 0 && currentSignature !== sentSignature) {
        // Cart contents genuinely differ from what was already confirmed —
        // this is a real new order, so clear the stale flags and show the form.
        sessionStorage.removeItem('fd_order_sent');
        sessionStorage.removeItem('fd_order_sent_signature');
        return;
      }
      // Either the cart is empty, or it exactly matches the order that was
      // just confirmed (e.g. the same products got re-added) — keep showing
      // the confirmed success page instead of reverting to the blank form.
      const wrap = document.getElementById('order-form-wrap');
      const success = document.getElementById('order-success');
      if (wrap && success) {
        wrap.style.display = 'none';
...
/* ─── CANVAS EMBERS (about hero) ─── */
(function initAboutCanvas(){
  const cv = document.getElementById('abcv');
  if (!cv) return;
  const ctx = cv.getContext('2d');
  if (!ctx) return;
  ...
  function drawBaseGlow() {
    const glow = ctx.createRadialGradient(
      cv.width * 0.3, cv.height * 0.6, 0,
      cv.width * 0.3, cv.height * 0.6, Math.max(cv.width, cv.height) * 0.6
    );
    glow.addColorStop(0, glowStart);
    glow.addColorStop(0.35, glowMid);
    glow.addColorStop(0.5, 'rgba(212,80,10,0.03)');
